...
public static class UI
{
    ...
    public static VNode Slider(float value, Action<float> onChange, float min = 0, float max = 1, Style.Style? style = null, float step = 0f, float thumbWidth = 14f, float thumbHeight = 14f, float thumbRadius = 7f, float trackHeight = 4f) =>
        SliderElement.Create(value, onChange, min, max, style, step, thumbWidth, thumbHeight, thumbRadius, trackHeight);

    /// <summary>
    /// Reports the pointer's normalized position inside the element while held — (0,0) top-left,
    /// (1,1) bottom-right. For position-valued controls like colour wheels and XY pads.
    /// </summary>
    public static VNode PointerArea(Action<UnityEngine.Vector2> onPointer, Style.Style? style = null, params VNode[] children) =>
        PointerAreaElement.Create(onPointer, style, children);
